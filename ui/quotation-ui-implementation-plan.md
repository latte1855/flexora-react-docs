...
| 狀態 | 描述 | 備註 |
| --- | --- | --- |
| ✅ | Quote Editor Page：3 欄 Layout、地址/ExtAttr 卡片、聯絡人/Owner Async Select、價目表卡片、預覽卡、行項卡片都已串 `/api/quotations/transactions` 一次儲存 | QuoteEditorPage 送出時會建立/更新 thread + revision + items，並顯示 preview/錯誤提示；整批 payload 已含 address/extAttrs。 |
| ✅ | Quick Create Drawer：客戶/Owner/價目表互動、缺價提示、附件拖曳、Workflow Diagram 按鈕、Timeline highlight | Drawer 與 Quote Editor 一樣改走 `/api/quotations/transactions`，建立草稿 / 新版 / 編輯皆共用 `focusThreadAfterChange` 返回 Detail。 |
| ✅ | Workflow Diagram 起訖節點、Timeline highlight、Attachments/Document download、Owner/Contact Async Select 共用化 | UI 與文件皆同步更新；IDF error key 映射完成。 |
| ✅ | Workflow Drawer / Pipeline 行為一致化 | Workflow Drawer 與 Pipeline 仍以 `triggerEvent` API 執行事件（暫不改走 transactions，待客戶驗收後再評估），但流程一律透過 `refreshAfterThreadChange` 同步刷新 Hub/Pipeline/Detail。 |
...
2. **Pipeline 拖拉**：拖曳卡片時呼叫 `POST /api/quotations/{id}/events/{eventCode}`；若失敗，UI 回滾並顯示錯誤（BadRequestAlert message）。
   - **Quick Create Drawer Flow**：使用者在 Drawer 填寫基本資料 → 呼叫 `POST /api/quotations/preview` 做行項/金額檢核 → 同一份 payload 再呼叫 **`POST /api/quotations/transactions`** 儲存（可建立草稿、建立新版本或覆寫草稿）。Drawer 需帶入行項、付款條件、Billing/Shipping Address Snapshot，並允許後續 Phase 5.1 擴充 ExtAttr/附件。
...
- **2025-11-21 新增**：Quick Create Drawer 延伸為「建立新版本」模式；Detail Drawer 增加「建立新版本」按鈕，會帶入 Thread/Revision/Line Items/ExtAttr 現有值並呼叫 `/api/quotations/transactions` 建立下一版（傳入 threadId 即可由後端產生新 revision）。
...
- 🔜 **下一步建議**
  ...
  3. **篩選 Saved Filters（`UI-03` 項下加強）**：在完成 URL query 同步後，下一步是提供條件收藏/命名、可一鍵套用，並顯示 ownerScope 提示；亦可將常用組合分享成連結。
...
| QTN-UI-03 | Quotation Hub List View （Table + Filter + Toolbar）（✅ 2025-11-15 初版完成，⏳ 進階 preset/filter 佈建）                                                                                                                                                                                                                                                                                        | 完成 DataTable、批次動作工具列、`quotePresets` + 進階篩選面板（owner/status/date/customer）、chips 單顆清除與 Pipeline 切換 tab，並支援 URL query 同步/視圖 chip。**TODO：Saved Filters（收藏條件）、owner scope tag 與 URL 分享快捷。**                                                                                                                                                                     | UI-01                    |
| QTN-UI-04 | Pipeline View + Drag & Drop（✅ 2025-11-15 初版完成）                                                                                                                                                                                                                                                                                                                 | 依狀態分欄、卡片顯示資訊；拖拉串接 Workflow API（SEND/APPROVE/REJECT…），即時提示成功/失敗。                                                                                                                                                                   | UI-02                    |
| QTN-UI-05 | Detail Page + Revision Timeline（進行中：2025-11-16 已完成摘要卡 / AddressCard / AttachmentList / ExtAttrPanel / Timeline / Quick Actions + 轉單 CTA；2025-11-17 新增 Overview / Items / Pricing / Workflow / Attachments Tabs、SalesOrder Links 卡片、Pricing 多幣別切換 + 稅額 tooltip + Discount History、Workflow 搜尋/匯出/Load More + 行動選單、附件下載/預覽） | 新增自訂詳細頁（/sales/quotations/:id）：摘要卡、地址、SalesOrder Link、狀態時間軸、Workflow Drawer 整合。**TODO：Timeline Segments（全部/特定版本/事件 chips）、流程圖/Timeline 高亮同步、Workflow Drawer 與 transaction API 整合**。                                                                                                                                                     | UI-03                    |
| QTN-UI-06 | Quotation Drawer（Create/Edit Wizard｜進行中：2025-11-16 新增步驟驗證、摘要區、Next Guard、共用 ExtAttrPanel）                                                                                                                                                                                                                                                        | Drawer Form（分 Step）、行項編輯表格、ExtAttr/附件區塊整合、號碼自動提示。已改用 `/api/quotations/transactions` 並與 Quote Editor 共用回寫流程。**TODO：後續加入行項匯入 / 表格編輯、以及 preview PDF。**                                                                                                                                                                                     | UI-04                    |
